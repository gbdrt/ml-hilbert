...
from src.tracking.ProofProgressTracker import (
    ProofTree,
    SubgoalNode,
    ProofStatus,
    ProofStrategy,
...
from src.tracking.resource_tracking import (
    TrackedAsyncLLMClient,
    TrackedAsyncProverLLM,
    StrategyTracker,
    MaxLLMCallsExceeded,
)
...
if PROOF_SYSTEM == "LEAN":
    ...
    from src.tools.string import (
        extract_lean_block as extract_proof_block,
        extract_all_lean_blocks as extract_all_proof_blocks,
    )
    from src.tools.lean_utils import (
    ...
    from src.tracking.resource_tracking import (
        TrackedAsyncLeanVerifier as TrackedAsyncVerifier,
    )
    from src.prompts.hilbert import (
    ...
    PROOF_FILE_EXTENSION = ".lean"
elif PROOF_SYSTEM == "ROCQ":
    from src.inference.AsyncRocqVerifier import AsyncRocqVerifier as AsyncVerifier
    from src.tools.string import (
        extract_coq_block as extract_proof_block,
        extract_all_coq_blocks as extract_all_proof_blocks,
    )
    ...
    from src.tracking.resource_tracking import (
        TrackedAsyncLeanVerifier as TrackedAsyncVerifier,
    )  # TODO: Create TrackedAsyncRocqVerifier
    from src.prompts_rocq.hilbert import (
        INFORMAL_LLM_CREATE_LEAN_SKETCH,
        INFORMAL_LLM_INFORMAL_PROOF_SKETCH,
        SEARCH_QUERY_PROMPT,
        SEARCH_ANSWER_PROMPT,
    ...
    from src.prompts_rocq.rocq_hints import GENERAL_HINTS, TACTIC_HINTS

    PROOF_FILE_EXTENSION = ".v"
else:
    raise ValueError(f"Unknown PROOF_SYSTEM: {PROOF_SYSTEM}. Must be 'LEAN' or 'ROCQ'")

# For backward compatibility, also import with original names
from src.tools.string import (
    extract_search_queries,
    extract_theorems_queries,
    extract_tag,
)
...
class HILBERTWorker:
    ...
    This class owns its own AsyncLLMClient and proof verifier instances and is designed
    ...
    The proof system (Lean or Rocq) is determined by the PROOF_SYSTEM constant at module level.
    """
    ...
    def __init__(
        ...
        Args:
            prover_llm: Async prover LLM for formal proof generation
            informal_llm_client: Async LLM client for informal reasoning
            proof_verifier: Async proof verifier (Lean or Rocq based on PROOF_SYSTEM)
            search_engine: Semantic search engine for theorem retrieval
        ...
        self.original_proof_verifier = proof_verifier
    ...
    async def close(self):
        ...
        if not self._closed:
            ...
            if hasattr(self.proof_verifier, "close"):
                await self.proof_verifier.close()
            self._closed = True

    async def _remove_import_statements(
        self, theorem: str, prev_prompt: str = None, prev_response: str = None
    ):
        """
        ...
        if not no_import_theorem:
            for i in range(self.proof_config.missing_tags_error_corrections):
                logger.info(100 * "*")
                logger.info(
                    "Trying to obtain the full theorem statement, attempt %d/%d",
                    i + 1,
                    self.proof_config.missing_tags_error_corrections,
                )
                logger.info(100 * "*")
                ...
                messages.append(
                    {"role": "user", "content": NO_THEOREM_STATEMENT_PROMPT}
                )
                ...
                extracted_theorem = extract_proof_block(response)
    ...
    async def _initialize_proof_tree(
        self, problem: str, problem_id: str = None
    ) -> None:
        ...
        async with self._proof_tree_lock:
            ...
            root_node = SubgoalNode(
                name=theorem_name, theorem=problem, depth=0, status=ProofStatus.SOLVING
            )
    ...
    def _save_proof_to_file(self, problem_id: str, header: str, proof: str):
        ...
        try:
            ...
            filename = f"{problem_id}{PROOF_FILE_EXTENSION}"
    ...
    async def _save_proof_tree_to_file(self, problem_id: str):
        ...
        try:
            async with self._proof_tree_lock:
                if not self.proof_tree:
                    logger.warning(
                        "No proof tree available to save for problem %s", problem_id
                    )
                    return
    ...
    async def _correct_response_for_missing_tags(
        self,
        original_prompt: str,
        ...
        Args:
            ...
            extract_function: Function to extract the required content (e.g., extract_tag, extract_proof_block)
    ...
    async def generate_proof_sketch(
        self, problem: str, useful_theorems: str
    ) -> Optional[str]:
        ...
        with (
            StrategyTracker(
                self.stats,
                StrategyType.SKETCH_GENERATION,
                0,
                extract_theorem_name(problem) or "main",
            )
            ...
            useful_theorems_section = self._format_useful_theorems_section(
                useful_theorems
            )
            input_prompt = INFORMAL_LLM_INFORMAL_PROOF_SKETCH.format(
                problem=problem, useful_theorems_section=useful_theorems_section
            )
            ...
            input_prompt = INFORMAL_LLM_CREATE_LEAN_SKETCH.format(
                problem=problem,
            ...
            logger.info(80 * "*")
            logger.info(
                "Printing Informal Proof Sketch Response Before Self Reflection"
            )
            logger.info(80 * "*")
            logger.info(proof_string)
            logger.info(80 * "*")

            parsed_proof = extract_proof_block(proof_string)
            if not parsed_proof:
            ...
            parsed_proof = await self._remove_import_statements(
                parsed_proof, input_prompt, proof_string
            )
            if not parsed_proof:
                return None
    ...
    async def correct_error_and_send_proof(
        self,
        error_message: str,
        useful_theorems: str,
        ...
        if type_of_correction == "sketch_outline":
            ...
            proof_correction_prompt = PROOF_SKETCH_CORRECTION_PROMPT.format(
            ...
            proof_correction_prompt = SUBGOAL_PROOF_CORRECTION_PROMPT.format(
                error_message=error_message,
                useful_theorems_section=useful_theorems_section,
            )
        elif type_of_correction == "sketch_completion":
            proof_correction_prompt = PROOF_SKETCH_ASSEMBLY_CORRECTION_PROMPT.format(
                error_message=error_message, lean_hints=GENERAL_HINTS
            )
        else:
        ...
        generated_proof = extract_proof_block(final_answer)
        ...
        if generated_proof:
            generated_proof = await self._remove_import_statements(
                generated_proof, proof_correction_prompt, final_answer
            )
            if not generated_proof:
                return None, messages_copy
        return generated_proof, messages_copy

    async def correct_error_in_theorem(
        self, error_message: str, header: str, problem: str
    ) -> Optional[str]:
        ...
        for i in range(self.proof_config.main_theorem_error_corrections):
            ...
            if useful_theorems:
                ...
                useful_theorems_str = ""
            proof_correction_prompt = THEOREM_CORRECTION_PROMPT.format(
                error_message=error_message,
            ...
            current_messages.append(
                {"role": "user", "content": proof_correction_prompt}
            )
            ...
            logger.info(final_answer)
            logger.info(100 * "*")
            generated_statement = extract_proof_block(final_answer)

            if not generated_statement:
                logger.info(100 * "$")
                logger.info(
                    "ERROR: Response did not have code block. Trying to correct it"
                )
                logger.info(100 * "$")
                # error correction loop
                ...
                if not generated_statement:
                    logger.info(100 * "$")
                    logger.info(
                        "ERROR: Response did not have code block. Returning None"
                    )
                    logger.info(100 * "$")
                    return None

            # Remove import statements
            generated_statement = await self._remove_import_statements(
                generated_statement, proof_correction_prompt, final_answer
            )
            if not generated_statement:
                logger.info(100 * "$")
                logger.info(
                    "ERROR: Could not obtain theorem statement without imports. Returning None"
                )
                logger.info(100 * "$")
                return None
            ...
            status, error_message = await self.proof_verifier.verify_proof(
    ...
    def _format_useful_theorems_section(self, useful_theorems: str) -> str:
        ...
        if not self.enable_retrieval or not useful_theorems.strip():
            ...
            return f"Here are a list of useful theorems to solve the problem:\n{useful_theorems}\n"
    ...
    async def search_and_select(
        self, problem: str = None, error_message: str = None
    ) -> str:
        ...
        if error_message is not None:
            ...
            search_prompt = ERROR_SEARCH_QUERY_PROMPT.format(
                error_message=error_message
            )
        else:
    ...
    async def _search_and_select_queries(
        self, problem: str, search_query_answer: str
    ) -> str:
        ...
        select_prompt = SEARCH_ANSWER_PROMPT.format(
            problem=problem, theorems=search_answer
        )
        ...
        useful_theorems = self.search_engine.search_by_name(
            selected_theorems, exact_match=True
        )
        logger.info(80 * "*")
    ...
    async def _single_attempt_prover_llm(
        self, theorem: str, header: str
    ) -> Optional[str]:
        ...
        if proof:
            ...
            result, error_message = await self.proof_verifier.verify_proof(
                header + proof,
    ...
    async def _attempt_proof_with_prover_llm(
        self, theorem: str, header: str
    ) -> Optional[str]:
        """
        ...
        args_for_each_job = [
            (theorem, header) for _ in range(self.proof_config.formal_proof_attempts)
        ]
        ...
        return await self._run_async_pool_and_get_first_truthy(
            self._single_attempt_prover_llm, args_for_each_job
        )

    async def _check_subgoal_is_correct(
        self, theorem: str
        ...
        correctness_prompt = DETERMINE_IF_CORRECT_SUBGOAL.format(
            problem=theorem, lean_hints=GENERAL_HINTS
        )
        ...
        correctness_prompt = CHECK_FOR_LEAN_ERRORS_PROMPT.format(
    ...
    def _setup_statistics_tracking(self, problem_id: str, problem_statement: str):
        ...
        self.stats = ProofStatistics(
            problem_id=problem_id or "unknown", problem_statement=problem_statement
        )

        # Wrap LLM clients with tracking wrappers
        self.prover_llm = TrackedAsyncProverLLM(
            self.original_prover_llm, self.stats, self.proof_config.max_prover_llm_calls
        )
        self.informal_llm_client = TrackedAsyncLLMClient(
            self.original_informal_llm_client,
            self.stats,
            LLMType.INFORMAL_LLM,
            self.proof_config.max_reasoner_llm_calls,
        )
        # Wrap lean verifier with tracking wrapper
        self.proof_verifier = TrackedAsyncVerifier(
            self.original_proof_verifier, self.stats
        )

    def _finalize_statistics(
        self, successfully_completed: bool, problem_id: str = None
    ):
    ...
    async def generate_single_proof(
        self, problem: str, header: str, problem_id: str = None
    ) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            if proof is not None:
                ...
                await self.update_proof_tree_strategy(
                    problem, ProofStrategy.SUBGOAL_DECOMP
                )
            ...
            self._finalize_statistics(
                success if "success" in locals() else False, problem_id
            )

    async def _single_subgoal_decomp_attempt(
        self, problem: str, header: str, depth: int, attempt_id: int = None
    ) -> Optional[str]:
        """
        ...
        generated_proof_sketch = await self.generate_proof_sketch(
            problem, useful_theorems
        )
        if not generated_proof_sketch:
            logger.info(100 * "$")
            logger.info(
                "ERROR: Response did not have a code block (attempt %d)",
                attempt_id,
            )
            logger.info(100 * "$")
        ...
        status, corrected_proof_sketch, extracted_theorems, proved_theorems = (
            await self._correct_proof_sketch_and_extract_theorems(
    ...
    async def subgoal_decomp(
        self, problem: str, header: str, depth: int = 0
    ) -> Tuple[bool, Optional[str]]:
        """
        ...
        if self.run_proof_attempts_sequentially:
            ...
            for args_tuple in args_for_each_job:
                logger.info(100 * "^")
                logger.info(
                    "STARTING Sketch ATTEMPT %d Valid sketch attempts so far: %d",
                    args_tuple[-1],
                    valid_sketch_attempts,
                )
                logger.info(100 * "^")
                ...
                result, generated_valid_proof_sketch = (
                    await self._single_subgoal_decomp_attempt(*args_tuple)
                )
    ...
    async def shallow_solve(
        ...
        with (
            StrategyTracker(
                self.stats,
                StrategyType.SHALLOW_SOLVE,
                0,
                extract_theorem_name(theorem) or "subgoal",
            )
            if self.enable_statistics
            else DummyContext()
        ):
            ...
            shallow_solve_prompt = SOLVE_SUBGOAL_PROMPT.format(
                problem=theorem,
                useful_theorems_section=useful_theorems_section,
                lean_hints=GENERAL_HINTS,
            ...
            proof = extract_proof_block(response)
            if not proof:
                logger.info(100 * "$")
                logger.info(
                    "ERROR: Response did not have a code block. Generating new response"
                )
                logger.info(100 * "$")
            ...
            # remove imports
            proof = await self._remove_import_statements(
                proof, shallow_solve_prompt, response
            )
            if not proof:
                logger.info(100 * "$")
                logger.info(
                    "ERROR: Response did not have theorem statement so abandoning."
                )
                logger.info(100 * "$")
                return False, None, None
            logger.info(80 * "*")
            ...
            if result:
                result, error_message = (
                    self._check_theorem_signature_match_and_get_error(theorem, proof)
                )
                if result:
    ...
    async def _augment_useful_theorems(
        self, error_message: str, problem: str, useful_theorems: str = ""
    ) -> str:
    ...
    async def _single_verify_and_correct_subgoal(
        ...
        for j in range(self.proof_config.subgoal_error_corrections):
            logger.info(80 * "-")
            logger.info(
                "Correction passes %d/%d",
                j + 1,
                self.proof_config.subgoal_error_corrections,
            )
            logger.info(80 * "-")
            logger.info("Error Message")
            ...
            if result:
                result, error_message = (
                    self._check_theorem_signature_match_and_get_error(
                        theorem, corrected_proof
                    )
                )
                if result:
    ...
    async def _verify_and_correct_subgoal(
        self, theorem: str, header: str, useful_theorems: str, depth: int
    ) -> Optional[str]:
        """
        ...
        args_for_each_job = [
            ...
            for _ in range(self.proof_config.parallel_subgoal_proof_attempts)
        ]
        result = await self._run_async_pool_and_get_first_truthy(
            self._single_verify_and_correct_subgoal, args_for_each_job
        )
    ...
    async def solve_subgoals(
        self,
        correct_proof_sketch: str,
        extracted_theorems: List[str],
        proved_theorems: Dict,
        header: str,
        useful_theorems: str,
        depth: int,
        ...
        for idx, theorem in enumerate(extracted_theorems):
            ...
            if theorem in proved_theorems:
                ...
                correct_proofs_by_job_name[extract_theorem_name(theorem)] = (
                    correct_proofs[idx]
                )
        ...
        for idx, proof in enumerate(correct_proofs):
            if proof is None:
                ...
                logger.info(80 * "*")
                logger.info(
                    "GIVING UP ON SOLVING THE SUBGOAL - ALL ATTEMPTS INCLUDING RECURSION FAILED"
                )
    ...
    async def extract_subgoals_from_sketch(
        self, proof_sketch: str, header: str
    ) -> List[str]:
        """
        ...
        extract_prompt = EXTRACT_SUBGOALS_FROM_SKETCH_PROMPT.format(
            proof_sketch=proof_sketch, lean_hints=GENERAL_HINTS
        )
        ...
        extracted_theorems = extract_all_proof_blocks(initial_response)
        if extracted_theorems is None:
            extracted_theorems = await self._correct_response_for_missing_tags(
                extract_prompt,
                initial_response,
                MISSING_LEAN_CODE_BLOCK_ERROR_PROMPT,
                extract_all_proof_blocks,
            )
            if extracted_theorems is None:
                return []
        ...
        status, missing_theorems = await self._check_and_extract_missing_subgoals(
            proof_sketch, extracted_theorems
        )
        if not status:
            logger.info(100 * "*")
            logger.info(
                "NOT ALL MISSING THEOREMS COULD BE EXTRACTED, ABANDONING CURRENT ATTEMPT"
            )
            logger.info(100 * "*")
            return []
        ...
        corrected_theorems = await self._verify_and_correct_syntax_errors_in_theorems(
            all_theorems, header
        )
        final_theorems = []
    ...
    async def _check_and_extract_missing_subgoals(
        self, original_sketch: str, extracted_theorems: List[str]
    ) -> List[str]:
        """
        ...
        for _ in range(self.proof_config.missing_subgoal_extraction_attempts):
            ...
            if len(missing_names) > 0:
                missing_names_message += (
                    "Missing have statement names: " + ", ".join(missing_names) + "\n"
                )
            # if len(extracted_names) < sorry_count:
            ...
            missing_theorems = extract_all_proof_blocks(missing_response)
            ...
            missing_names = [
                name for name in all_have_names if name not in extracted_names
            ]
    ...
    async def _refine_proof_sketch(
        self, proof_sketch: str, justification: str
    ) -> Optional[str]:
        """
        ...
        correct_sketch_prompt = CORRECT_SKETCH_BASED_ON_INCORRECT_SUBGOAL.format(
            proof_sketch=proof_sketch, issues=justification, lean_hints=GENERAL_HINTS
        )
        ...
        proof_sketch_response = extract_proof_block(proof_sketch_response_str)
        ...
        if proof_sketch_response:
            proof_sketch_response = await self._remove_import_statements(
                proof_sketch_response, correct_sketch_prompt, proof_sketch_response_str
            )
        return proof_sketch_response

    def _check_theorem_signature_match_and_get_error(
        self, problem: str, candidate_proof: str
    ):
        if not check_theorem_signature_match(candidate_proof, problem):
            ...
            error_message = THEOREM_SIGNATURE_MISMATCH_PROMPT.format(
    ...
    async def _compile_and_correct_proof_sketch(
        self, proof_sketch: str, header: str, useful_theorems: str, problem: str
    ) -> Tuple[bool, Optional[str]]:
        """
        ...
        if result:
            ...
            result, error_message = self._check_theorem_signature_match_and_get_error(
                problem, corrected_proof_sketch
            )
        ...
        if not result:
            ...
            for j in range(self.proof_config.main_theorem_error_corrections):
                ...
                logger.info(
                    "Correction passes %d/%d",
                    j + 1,
                    self.proof_config.main_theorem_error_corrections,
                )
                logger.info(80 * "-")
                logger.info("Error Message: %s", error_message)
                logger.info(80 * "-")
                ...
                logger.info(useful_theorems)
                logger.info(80 * "-")
                generated_proof, messages = await self.correct_error_and_send_proof(
                    error_message=error_message,
                    useful_theorems=useful_theorems,
                    messages=messages,
                ...
                if not generated_proof:
                    logger.info("ERROR! The sketch did not have a code block")
                    return False, None
                # extract sketch
    ...
    async def verify_and_correct_proof_sketch_with_theorems(
        self, proof_sketch: str, all_theorems: str, header: str, problem: str
    ):
        """
        ...
        logger.info("ERROR MESSAGE: %s", error_message)
        if status:
            status, error_message = self._check_theorem_signature_match_and_get_error(
        ...
        for j in range(self.proof_config.main_theorem_error_corrections):
            logger.info(80 * "-")
            logger.info(
                "Proof sketch correction passes %d/%d",
                j + 1,
                self.proof_config.main_theorem_error_corrections,
            )
            logger.info(80 * "-")
            ...
            if not corrected_proof:
                logger.info("ERROR: Response did not have a code block")
                continue
    ...
    async def _use_sketch_and_theorems_to_generate_proof(
        self, proof_sketch: str, theorems: List[str], header: str, problem: str
    ):
        """
        ...
        with (
            ...
            get_sketch_prompt = USE_SKETCH_AND_THEOREMS_TO_PROVE.format(
                theorems_string=all_theorems, proof_sketch=proof_sketch
            )
            response = await self.informal_llm_client.simple_chat(
                get_sketch_prompt,
                max_tokens=32768,
            ...
            logger.info(response)
            logger.info(100 * "*")
            completed_proof_sketch = extract_proof_block(response)
            if not completed_proof_sketch:
                ...
                if not completed_proof_sketch:
                    ...
                    return None
            # remove import statements
            completed_proof_sketch = await self._remove_import_statements(
                completed_proof_sketch, get_sketch_prompt, response
            )
            # verify and correct errors in sketch
            corrected_proof_sketch = (
                await self.verify_and_correct_proof_sketch_with_theorems(
                    completed_proof_sketch, all_theorems, header, problem
                )
            )
    ...
    async def _syntax_check_and_correct_single_theorem(self, theorem: str, header: str):
        ...
        if status:
            ...
            logger.info(error_message)
            logger.info(80 * "*")
            theorem = await self.correct_error_in_theorem(
                error_message, header, theorem
            )
    ...
    async def _verify_and_correct_syntax_errors_in_theorems(
        self, theorems, header: str
    ):
    ...
    async def _process_single_theorem(
        self,
        theorem: str,
        header: str,
        corrected_theorems: List[str],
        proved_theorems: Dict,
        ...
        if subgoal_status:
            ...
            theorem_name = extract_theorem_name(theorem)
            full_justification = (
                f"Sub goal {theorem_name} has an issue:\n{justification}"
            )
            return False, theorem, None, full_justification

    async def _correct_theorems_from_sketch(
        self,
        extracted_theorems: List[str],
        header: str,
        corrected_theorems: List[str] = [],
        proved_theorems: Dict = {},
    ) -> Tuple[bool, Optional[List[str]], Optional[str], Optional[Dict]]:
        """
    ...
    async def _correct_proof_sketch_and_extract_theorems(
        self, proof_sketch: str, header: str, problem: str, useful_theorems: str
        ...
        for k in range(correction_budget + 1):
            ...
            status, corrected_proof_sketch = (
                await self._compile_and_correct_proof_sketch(
                    proof_sketch, header, useful_theorems, problem
                )
            )
            ...
            extracted_theorems = await self.extract_subgoals_from_sketch(
                proof_sketch=corrected_proof_sketch, header=header
            )
            ...
            extracted_theorems = self.rename_theorems(
                extracted_theorems, parent_theorem_name
            )

            # check if you can make a full proof with them
            completed_proof_sketch = (
                await self._use_sketch_and_theorems_to_generate_proof(
                    corrected_proof_sketch, extracted_theorems, header, problem
                )
            )
            ...
            status, corrected_theorems, justification, proved_theorems = (
                await self._correct_theorems_from_sketch(
                    extracted_theorems, header, corrected_theorems, proved_theorems
                )
            )
            ...
            if status:
                ...
                if k != correction_budget:
                    proof_sketch = await self._refine_proof_sketch(
                        corrected_proof_sketch, justification
                    )
